...
// New circuits missing from multiviewer get an outline built by the proxy from
// a real GPS lap — it needs a completed lap first, so poll until it appears.
const PROXY_MAP_RETRY_MS = 30_000;
...
export default function TrackMap({
  ...
  useEffect(() => {
    ...
    const fetchMap = async () => {
      ...
      // Fallback: outline generated by the proxy from live GPS data
      // (circuits multiviewer doesn't know yet, e.g. Madring 2026).
      if (!data) {
        try {
          const response = await fetch(`${PROXY_URL}/api/circuit/${circuitKey}`);
          if (response.ok) data = await response.json();
        } catch {
          // Proxy unreachable — handled by the retry below
        }
      }
      ...
      if (data) {
        ...
        setMapData(null);
        retryTimer = setTimeout(fetchMap, PROXY_MAP_RETRY_MS);
      }
      setLoading(false);
    };
